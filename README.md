...
flowchart LR
	...
	Hosting -->|"/api/todos/**<br/>/api/web-pages/**<br/>/api/push-notifications/**<br/>/api/auth/google/**<br/>/api/auth/apple/challenges<br/>/api/auth/apple/custom-token"| CloudRun["Cloud Run<br/>http-api"]
...
| Cloud Run `http-api` | `nestjs-api` | Todo·WebPage·PushNotification 요청의 Firebase ID Token 검증과 삭제, Google 인증, Apple challenge/custom-token 처리 |
...
Staging Hosting에서는 `/api/todos/**`, `/api/web-pages/**`, `/api/push-notifications/**`, `/api/auth/google/**`, `/api/auth/apple/challenges`, `/api/auth/apple/custom-token`을 Cloud Run `http-api`가 우선 처리하고 GitHub callback과 나머지 `/api/**`는 기존 Functions `api`가 계속 처리합니다. `/api/auth/apple/account-link`, `/api/auth/apple/access-token`, `/api/auth/apple/refresh-token`은 아직 Functions `api`에서 처리됩니다.
...
Google 인증은 서버 callback route를 사용하지 않으며 Staging에서는 `/api/auth/google/**` Hosting rewrite로 Cloud Run에 전달됩니다. Apple 인증은 `#87`에서 `/api/auth/apple/challenges`, `/api/auth/apple/custom-token`만 Cloud Run으로 이전했고, `#88` 대상인 `/api/auth/apple/account-link`, `/api/auth/apple/access-token`, `/api/auth/apple/refresh-token`은 Functions `api`로 유지됩니다. GitHub 인증은 기존 `callbackURL`과 Hosting rewrite를 계속 사용합니다.
