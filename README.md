...
flowchart LR
	...
	Hosting -->|"/api/todos/**<br/>/api/web-pages/**<br/>/api/push-notifications/**<br/>/api/auth/google/**"| CloudRun["Cloud Run<br/>http-api"]
...
| Cloud Run `http-api` | `nestjs-api` | Firebase ID Token을 검증하고 Todo·WebPage·PushNotification 삭제와 Google 인증 처리 |
...
Staging Hosting에서는 `/api/todos/**`, `/api/web-pages/**`, `/api/push-notifications/**`, `/api/auth/google/**`를 Cloud Run `http-api`가 우선 처리하고 GitHub callback과 나머지 `/api/**`는 기존 Functions `api`가 계속 처리합니다.
...
Google 인증은 서버 callback route를 사용하지 않으며 Staging에서는 `/api/auth/google/**` Hosting rewrite로 Cloud Run에 전달됩니다. GitHub 인증은 기존 `callbackURL`과 Hosting rewrite를 계속 사용합니다.
