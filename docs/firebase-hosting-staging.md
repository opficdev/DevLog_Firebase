...
6. `/api/auth/github/sign-in-sessions` → Cloud Run `http-api`
7. `/api/auth/github/callback` → Cloud Run `http-api`
8. `/api/auth/github/custom-token` → Cloud Run `http-api`
9. `/api/auth/github/account-link-sessions` → Cloud Run `http-api`
10. `/api/**` → Functions `api`
...
유효한 일회용 인증 값이 필요한 Apple challenge, custom token, account-link, account-link 해제, access-token 갱신·폐기 및 refresh-token 성공 경로와 GitHub 로그인·계정 연결 성공 경로는 Staging 앱에서 확인합니다. 성공 응답과 최신 `http-api` revision 로그를 함께 확인하며 Token과 인증 코드는 로그와 shell history에 남기지 않습니다.
...
Todo, WebPage, PushNotification, Google·Apple 인증과 이전한 네 GitHub 인증 요청은 최신 `http-api` revision에서 확인되어야 합니다. GitHub 계정 연결 해제와 access token 제거를 포함한 나머지 요청은 Functions `api`에서 확인되어야 합니다.

## GitHub 인증 라우팅 복구

Functions `api`가 GitHub session·callback·custom token route 제거가 포함된 소스로 배포되었는지 먼저 확인합니다. 배포 이력이 불분명하면 route가 제거된 상태로 판단합니다.

### 기존 Functions route가 배포된 기간

이 변경에서는 Functions를 배포하지 않으므로 GitHub route 제거가 포함된 소스로 Functions `api`를 배포하기 전까지는 기존 route가 배포된 상태입니다. 이 기간에만 `firebase.json`의 `devlog-staging` 블록에서 이전한 네 GitHub Cloud Run rewrite를 제거해 기존 Functions로 되돌릴 수 있습니다. GitHub 계정 연결 해제와 access token 제거는 계속 `/api/**`를 통해 Functions로 전달됩니다.
...
앞 절의 네 GitHub 인증 요청을 다시 실행하고 Functions `api` 로그에서 확인합니다. Cloud Run 서비스와 revision은 삭제하거나 변경하지 않습니다.

### Functions route 제거 배포 이후

Functions `api`가 GitHub route 제거가 포함된 소스로 배포된 뒤에는 네 Cloud Run rewrite를 제거하는 복구를 사용하지 않습니다. 요청이 `/api/**`를 통해 Functions로 전달되어 `404`를 반환하기 때문입니다.
...
4. `/api/auth/github/sign-in-sessions` → Cloud Run `http-api`
5. `/api/auth/github/callback` → Cloud Run `http-api`
6. `/api/auth/github/custom-token` → Cloud Run `http-api`
7. `/api/auth/github/account-link-sessions` → Cloud Run `http-api`
8. `/api/**` → Functions `api`
